...
/**
 * 
 * this baseline with no Heuristic
 * cover check with Cartesian (check every two)
 * @author Sea
 *
 *
 */

public class Baseline2 extends Generator{
	
	public Baseline2(Concept concept) {
		super(concept);
	}
	...
	{
		System.out.println("-----------Baseline2------genPattern----------");
...
//							if(!this.trie.query(pt))
	...
	public void solution() {
		...
		for(Sdsp s1:sdsp)
		{
			for(Sdsp s2:sdsp)
			{
				if(checkCover(s2.pattern, s1.pattern))
				{
					flag = true;
					break;
				}
...
//		this.dsp = this.trie.traver();
	...
	// check whether s1 cover s2
	private boolean checkCover(List<String> s1, List<String> s2)
	{
		if(s1.size() > s2.size())
			return false;
		for(int i = 0; i < s1.size(); i++)
		{
			...
			for(int j = 0; j < s2.size(); j++)
			{
				...
				if(contain(e1, e2))
				{
					if(checkCover(s1.subList(i+1, s1.size()), s2.subList(j+1, s2.size())))
						return true;
				}
			}
		}
